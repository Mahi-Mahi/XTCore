...
var xtoid = new Array,
	...
	xd = document,
	xtg = navigator,
	xtv = (xw.xtczv != null) ? '42002-' + xw.xtczv : '42002',
	xt1 = xw.xtdmc ? ';domain=' + xw.xtdmc : (xt1 != '') ? ';domain=' + xw.xt1 : '',
	xt6 = (xw.xtnv != null) ? xw.xtnv : xd,
	xt7 = (xw.xtsdi != null) ? xw.xtsdi : (xw.xtsd != null) ? xw.xtsd + '.xiti.com/hit.xiti' + (xtfirst ? 'f' : '') : (((xd.location.protocol == 'https:') ? 'https://logs' : 'http://logp') + '.xiti.com/hit.xiti' + (xtfirst ? 'f' : '')),
...
function xtserial(obj) {
	...
	if (t != "object" || obj === null) {
		...
		for (n in obj) {
			...
			else if (t == "object" && v !== null) v = xtserial(v);
...
function xtf3(param, chba, a) {
	...
	if ((chba == null) || (chba == xtud)) {
		xturl = xtclURL(xtdeb.toLowerCase().replace(/%3d/g, '='))
	} else {
		xturl = chba
	}
	...
	if (xtpos > 0) {
		...
		var pos = mq.search(reg);
		if (pos == -1) pos = mq.length;
		if (a == 1) {
...
xtfirst = ((xtg.userAgent.indexOf('Safari') != -1 && xtg.userAgent.indexOf('Chrome') < 0) || xtg.userAgent.indexOf('iPhone') != -1 || xtg.userAgent.indexOf('iPod') != -1 || xtg.userAgent.indexOf('iPad') != -1 || xtfirst);
if (xtfirst) {
...
function xt_rm(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14) {
	var rmprm = '&p=' + x3 + '&s2=' + x2 + '&type=' + x1 + '&a=' + x4 + '&m5=' + x11 + '&m6=' + x12;
	rmprm += ((x5 != null) && (x5 != '0')) ? '&' + x5 : '';
	rmprm += ((x7 != null) && (x4 != 'pause') && (x4 != 'stop')) ? '&m1=' + x7 + '&' + x8 + '&m3=' + x9 + '&m4=' + x10 + '&m7=' + x13 + '&m8=' + x14 + '&prich=' + xtp + '&s2rich=' + xw.xtn2 : '';
	rmprm += ((x6 != null) && (x6 != '0') && (x7 != null)) ? '&rfsh=' + x6 : '';
	xtf4(x1, rmprm);
	if ((x6 != null) && (x6 != '0') && ((x4 == 'play') || (x4 == 'play&buf=1') || (x4 == 'refresh'))) {
		xtrmdl = (Math.floor(x6) > 1500) ? 1500000 : (Math.floor(x6) < 5) ? 5000 : Math.floor(x6) * 1000;
		xtoid[x1] = xw.setTimeout("xt_rm('" + x1 + "','" + x2 + "','" + x3 + "','refresh','0','" + x6 + "',null,'" + x8 + "','" + x9 + "','" + x10 + "','" + x11 + "','" + x12 + "')", xtrmdl);
...
function xtf4(x1, x2, x3, x4) {
	if (((xtclzone == 0) || (xtclzone == 3) || (x1 != 'C')) && (x1 != 'P')) {
		...
		var xt22 = new Date();
		xt_im = xt7 + '?' + xt8b + x2 + '&hl=' + xt22.getHours() + 'x' + xt22.getMinutes() + 'x' + xt22.getSeconds() + (xtfirst ? '&idclient=' + xt40 : '');
...
function xt_ParseUrl(hit, xtch, xtrefP, thit) {
	...
	if (xtch.length > 0) {
		...
		if (xterr == 0) {
			...
			if (xt38 != '') {
				var stc = '&stc=';
				if ((stc.length + xt38.length + xtch.length) < xtlg) {
					tabUrl[i] += (stc + xt38);
				} else {
					i++;
					xt38 = stc + xt38;
					while ((xt38.length > xtlg) && (xtch_prec != xt38)) {
						xtch_prec = xt38;
						var xsep = ',';
						if (xt38.lastIndexOf(xsep, xtlg) <= 5) {
							xterr = 1;
						}
						if (xterr == 1) {
							tabUrl[i] = xt38.substring(0, 1600) + '&mherr=1';
						} else {
							tabUrl[i] = xt38.substring(0, xt38.lastIndexOf(xsep, xtlg));
							xt38 = stc + xt38.substring(xt38.lastIndexOf(xsep, xtlg), xt38.length);
							i++;
							xtlg = 1600;
						}
					}
...
function xt_ParseUrl3(str1, str2, nt, ntg, name, sep, idp) {
	...
	if (str2.length > lim) {
		...
		mh = '&mh=' + nt + '-' + ntg + '-' + idp;
		if (hit != '') {
			xt_imgc.src = str1 + '&idp=' + idp + mh + hit;
			xt_imgc.onload = function() {
				xt_imgc.onload = null
			}
		} else {
			xt_imgc.src = str1 + '&idp=' + idp + mh + str2.substring(0, lim) + '&mherr=1';
			xt_imgc.onload = function() {
				xt_imgc.onload = null
			};
			return
		...
		xt_imgc.src = (nt == 1) ? str1 + '&idp=' + idp + str2 : str1 + '&idp=' + idp + mh + str2;
		xt_imgc.onload = function() {
			xt_imgc.onload = null
		}
	}
...
if ((xt8 != 0) || (xt36 != 0) || (xt11)) {
	...
	var xtpm = 'xtor' + xt8,
		...
		xtpgt = (xtgord == '1') ? '&pgt=' + xtf1('xtord') : ((xtgo == '1') && (xt24 != null)) ? '&pgt=' + xt24 : '',
		xto = (xt10 != null) ? xt10 : ((xt24 != null) && (xtgo == '0')) ? xt24 : (!xt11) ? xtord : null;
	xto = ((xtocl.indexOf('$' + xto + '$') < 0) || (xtocl == '$')) ? xto : null;
	var xtock = (xtgo == '0') ? xto : (xtgord == '2') ? xtf1('xtord') : (xtgo == '2') ? xt24 : null;
	if (xtock != null) {
	...
	if (Xt_r != null) Xt_r = Xt_r.substring(0, 1000);
	if (!xt11) {
		...
		Xt_param += xt33 + xtresr + xt16 + ((xtide != null) ? '&ide=' + xtide : '');
